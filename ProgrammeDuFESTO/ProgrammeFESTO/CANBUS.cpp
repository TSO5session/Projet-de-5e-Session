...
EcrireRegistreMCP2515(MCP2515_CANCTRL, 0x03);// Mode LoopBak 03 ou 02?  

UC Identifiant       = 0;  
UC LongueurDuMessage = 0;  // Pour lire la longeur du message

LongueurDuMessage = WriteMPC(MCP2515_RXB0DLC);
LongueurDuMessage = LongueurDuMessage & 0x00FF; // Bit d'obtemtion de longueur

Low5              = WriteMPC(MCP2515_RXB0SIDL); // Lecture des 3 bits de l'octet du bas
High3             = WriteMPC(MCP2515_RXB0SIDH); // Lecture des 8 bits de l'octet du haut
...
EcrireRegistreMCP2515(MCP2515_CANCTRL, 0x07);   // Retour au mode normal
return(Identifiant);
}
...
// Envoi de la commande de   RESET  , qui consiste en l'envoi de l'octet   0xC0  , par le bus SPI. 
   WriteMPC(3, MCP2515_RESET, 0x00, 0x00);
...
{
WriteMPC(3, MCP2515_WRITE, Address, Data);   /* Envoi de 3 octets :
...
WriteMPC(1, MCP2515_RTS_TX0, 0x00, 0x00);
Delai(10);  
}
